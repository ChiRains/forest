...
public interface MerchantEvaluationMapper {
    ...
    @Insert("insert into `${table_name}`(`id`,`evaluationId`,`merchantId`,`merchandiseId`,`evaluationTime`,`content`)" + " values(#{id},#{evaluationId},#{merchantId},#{merchandiseId},#{evaluationTime},#{content})")
    public void insert(MerchantEvaluation merchantEvaluation);

    @Select("select * from `${table_name}` where `id`=#{id}")
    public MerchantEvaluation get(Long id);

    @Update("update `${table_name}` set `evaluationId`=#{evaluationId},`merchantId`=#{merchantId},`merchandiseId`=#{merchandiseId},`evaluationTime`=#{evaluationTime},`content`=#{content} where `id`=#{id}")
