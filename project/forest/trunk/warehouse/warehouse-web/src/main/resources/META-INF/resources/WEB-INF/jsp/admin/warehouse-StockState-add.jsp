...
<div class="row">
    <div class="col-xs-12">
        <!-- PAGE CONTENT BEGINS -->
        <form id="model-form" class="form-horizontal"  role="form" action="/admin/stockState/add.do">
            <!-- #section:elements.form -->
            <input type="hidden" id="toStoreId" name="toStoreId" value="${storeMember.storeId}"/>
            
            <div class="space-4"></div>
            <div class="form-group">
                <label class="col-sm-3 control-label no-padding-right" for="formStoreId"> 出货门店 </label>
                <div class="col-sm-9">
					<span class="col-sm-5 no-padding block input-icon input-icon-right mr10">
						<select class="chosen-select" id="formStoreId" name="formStoreId" data-placeholder="请选择分类">
	                	<option value="">请选择门店</option>
		                 	<c:forEach items="${storeList}" var="store" varStatus="current">
		                   		 <option value="${store.id}"
		                   		 <c:if test="${store.id eq query.storeId}">
			                   		 selected
	                			 </c:if>>${store.name}</option>
		                   	</c:forEach>
	                	</select>
                    ...
                    <table class="table table-striped table-bordered table-hover kv-table">
                        ...
                        <c:forEach items="${voList}" var="item" varStatus="current">
                            <tr class="template kv-table-row">
                                ...
                                <td><input type="text" name="numbers[${current.index}]" value="0"></td>
                                <td class="del-column">
            ...
            <div class="clearfix form-actions">
                <div class="col-md-offset-3 col-md-9">
                    ...
                    &nbsp; &nbsp; &nbsp;
                    <button class="btn" type="button" onclick="window.location.href='#admin/merchandiseStockState/list'">
        ...
        jQuery(function ($) {
            ...
            function reSetIndex() {
                $("#productList").children().each(function (i) {
                    ...
                    $(this).find("input[name^='numbers']").attr("name", $(this).find("input[name^='numbers']").attr("name").replace(/\[.*?\]/g, "[" + i + "]"));
                });
            }
            ...
            $(".add-row-trigger").on("click", function () {
                ...
                BootstrapDialog.show({
                    title: "商品列表",
                    message: $('<div></div>').load('/admin/stockState/selectProductList.do?formStoreId=' + formStoreId),
                            ...
                            if (obj.hasClass("add-btn")) {
                                ...
                                if (id && name) {
                                    ...
                                    count >= 0 && $("#productList").append(
                                            ...
                                            '<td><input type="text" name="numbers[]" value="0" class="config-value"></td>' +
