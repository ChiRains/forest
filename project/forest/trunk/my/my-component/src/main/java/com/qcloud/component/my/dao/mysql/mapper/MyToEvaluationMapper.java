...
public interface MyToEvaluationMapper {
	...
	@Insert("insert into `my_my_to_evaluation`(`id`,`userId`,`unifiedMerchandiseId`,`merchandiseId`,`name`,`image`,`discount`,`merchantId`,`orderId`,`subOrderId`,`orderItemId`,`orderDate`,`orderNumber`,`signDate`,`orderItemDetailId`)"
			+ " values(#{id},#{userId},#{unifiedMerchandiseId},#{merchandiseId},#{name},#{image},#{discount},#{merchantId},#{orderId},#{subOrderId},#{orderItemId},#{orderDate},#{orderNumber},#{signDate},#{orderItemDetailId})")
