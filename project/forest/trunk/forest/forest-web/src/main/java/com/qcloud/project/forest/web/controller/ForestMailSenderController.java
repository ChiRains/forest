...
@Controller
@RequestMapping(value = "/" + ForestMailSenderController.DIR)
public class ForestMailSenderController {

    private final String           MAIL_SENDER_INFO = "mailSenderInfo";

    public final static String     DIR              = "/forestMailSender";
    ...
    public FrontAjaxView sendMail(String email) {
        ...
        if (SmsMessageStateType.NUMBER_LIMIT.equals(type)) {
            throw new PublicdataException("邮件发送失败,已经超出今天发送最大数量.");
        }
        if (SmsMessageStateType.TIME_LIMIT.equals(type)) {
            throw new PublicdataException("邮件发送失败,发送太频繁,请稍等.");
        }
        StringBuilder sb = new StringBuilder();
        sb.append("您好！您正在绑定大参林线上公众号的邮箱，本次请求的验证码为：{code}（请在{minute}分钟内完成验证，逾时该验证码将会失效。）");
    ...
    @PiratesApp
    @RequestMapping
    public FrontAjaxView editBingdingEMail(HttpServletRequest request, String email, String code) {
