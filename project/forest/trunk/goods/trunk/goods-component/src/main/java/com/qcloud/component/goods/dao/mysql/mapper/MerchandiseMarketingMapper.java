...
public interface MerchandiseMarketingMapper {
	...
	@Insert("insert into `goods_merchandise_marketing`(`id`,`merchandiseItemId`,`unifiedMerchandiseId`,`purchase`,`discount`,`price`,`stock`,`sence`,`updateTime`,`order`,`currencyType`,`activityId`,`enable`,`name`,`sysCode`)"
			+ " values(#{id},#{merchandiseItemId},#{unifiedMerchandiseId},#{purchase},#{discount},#{price},#{stock},#{sence},#{updateTime},#{order},#{currencyType},#{activityId},#{enable},#{name},#{sysCode})")
	...
	@Update("update `goods_merchandise_marketing` set `merchandiseItemId`=#{merchandiseItemId},`unifiedMerchandiseId`=#{unifiedMerchandiseId},`purchase`=#{purchase},`discount`=#{discount},`price`=#{price},`stock`=#{stock},`sence`=#{sence},`updateTime`=#{updateTime},`order`=#{order},`currencyType`=#{currencyType},`activityId`=#{activityId},`enable`=#{enable},`name`=#{name},`sysCode`=#{sysCode} where `id`=#{id}")
