...
public interface SlideMapper {
    ...
    @Insert("insert into `marketing_slide`(`id`,`clickUrl`,`image`,`sence`,`orderNum`,`desc`,`startDate`,`endDate`,`enable`,`title`)" + " values(#{id},#{clickUrl},#{image},#{sence},#{orderNum},#{desc},#{startDate},#{endDate},#{enable},#{title})")
    public void insert(Slide slide);

    @Select("select * from `marketing_slide` where `id`=#{id}")
    public Slide get(Long id);

    @Update("update `marketing_slide` set `clickUrl`=#{clickUrl},`image`=#{image},`sence`=#{sence},`orderNum`=#{orderNum},`desc`=#{desc},`startDate`=#{startDate},`endDate`=#{endDate},`enable`=#{enable},`title`=#{title} where `id`=#{id}")
