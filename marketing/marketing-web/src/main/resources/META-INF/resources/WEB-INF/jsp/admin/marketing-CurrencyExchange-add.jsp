...
<div class="row">
    <div class="col-xs-12">
        <!-- PAGE CONTENT BEGINS -->
        <form id="model-form" class="form-horizontal"  role="form" action="/admin/currencyExchange/addExchangeMerchandise.do">
            <!-- #section:elements.form -->
            
            <div class="space-4"></div>
            <div class="form-group">
            	<label class="col-sm-6" style="color:red;"> 注: 下表所示原价为消费币价格，其兑换率为${bit}/100人民币</label>
            </div>
        ...
        jQuery(function ($) {
            ...
            function reSetIndex() {
                $("#productList").children().each(function (i) {
                    ...
                    $(this).find("input[name^='purchase']").attr("name", $(this).find("input[name^='purchase']").attr("name").replace(/\[.*?\]/g, "[" + i + "]"));
                	$(this).find("input[name^='price']").attr("name", $(this).find("input[name^='price']").attr("name").replace(/\[.*?\]/g, "[" + i + "]"));
                	...
                	$(this).find("input[name^='oldStock']").attr("name", $(this).find("input[name^='oldStock']").attr("name").replace(/\[.*?\]/g, "[" + i + "]"));
            ...
            $(".add-row-trigger").on("click", function () {
                BootstrapDialog.show({
                    title: "商品列表",
                    message: $('<div></div>').load('/admin/merchandiseItem/listForAdmin.do?listIds=${listIds}'),
                            ...
                            if (obj.hasClass("add-btn")) {
                                ...
                                if (name) {
                                    ...
                                    count >= 0 && $("#productList").append(
                                            ...
                                            '<td><input type="text" name="discount[]" value="' + discount + '"  class="config-value"></td>' +
                                            '<td><input type="text" name="price[]" value="' + price + '" readonly  class="config-value"></td>' +
                                            '<td><input type="text" name="stock[]" value="' + stock + '"  class="config-value">'+
                                            '<input type="hidden" name="oldStock[]" value="' + stock + '"  class="config-value">'+
                                            '</td>' +
                                            '<td><input type="text" name="orderNums[]" value="0" class="config-value"></td>' +
