...
<div class="row">
    <div class="col-xs-12">
        <!-- PAGE CONTENT BEGINS -->
        <form id="model-form" class="form-horizontal"  role="form" action="/admin/classifyAttribute/editClassifyAttr.do">
            <!-- #section:elements.form -->
            
            <input type="hidden" name="attributeId" value="${attribute.id}">
			<input type="hidden" name="id" value="${result.id}">
			 <input type="hidden" name="classifyId" value="${attribute.classifyId}">
            ...
            <div class="form-group">
                ...
                <div class="col-sm-9">
					<span class="col-sm-5 no-padding block input-icon input-icon-right mr10">
						<input type="text" class="width-100" maxlength="20" id="name" name="name" placeholder="名称" value="${result.name}"/>
            ...
            <div class="form-group">
                ...
                <div class="col-sm-9">
					<span class="col-sm-5 no-padding block input-icon input-icon-right mr10">
						<input type="text" onKeyUp="value=value.replace(/[^\a-\z\A-\Z]/g,'')" class="width-100" maxlength="25" id="code" name="code" placeholder="编码" value="${result.code}"/>
						<i class="ace-icon fa"></i>
					</span>
            ...
            <div class="form-group">
                ...
                <div class="col-sm-9">
					<span class="col-sm-5 no-padding block input-icon input-icon-right mr10">
						<input type="text" class="width-100" maxlength="100" id="remark" name="remark" placeholder="说明,可区分相同的名称" value="${result.remark}"/>
						<i class="ace-icon fa"></i>
					</span>
                </div>
            </div>  
                ...
                <div class="col-sm-9">
					<span class="col-sm-5 no-padding block input-icon input-icon-right mr10">
						<select class="form-control" id="valueType" name="valueType">
							  <option <c:if test="${result.valueType==1}">selected</c:if> value="1">输入框</option>
							  <option <c:if test="${result.valueType==2}">selected</c:if> value="2">下拉框</option>
							  <option <c:if test="${result.valueType==3}">selected</c:if> value="3">单选框</option>
							  <option <c:if test="${result.valueType==4}">selected</c:if> value="4">复选框</option>
							  <option <c:if test="${result.valueType==5}">selected</c:if> value="5">富文本</option>
							  <option <c:if test="${result.valueType==6}">selected</c:if> value="6">文本域</option>
			            </select> 	
					</span>
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <div class="col-sm-9">
					<span class="col-sm-5 no-padding block input-icon input-icon-right mr10">
						<input type="text" class="width-100" maxlength="20" id="sort" name="sort" placeholder="排序" value="${attribute.sort}"/>
						<i class="ace-icon fa"></i>
					</span>
                </div>
            </div>
