        ...
        <div>
            <div class="dataTables_wrapper form-inline no-footer">
				...
				<table class="table table-striped table-bordered table-hover dataTable no-footer">
					...
					<tbody>
					<tr>
						<td style="width: 200px;">物流公司：</td><td style="width: 300px;color: green;">${order.logisticsCompany}</td>
						<td style="width: 200px;">物流查询号：</td><td style="width: 300px;color: green;">${order.logisticsNumber}</td>
					</tr>
				</tbody>
				</table>
				...
				<table class="table table-striped table-bordered table-hover dataTable no-footer">
					<caption class="text-left table-header btn-info">退货商品列表&nbsp;&nbsp;&nbsp;&nbsp;
					</caption>
					<thead> 
						<tr role="row">
                ...
                <div class="row">
                    <div class="col-xs-12">
                        <div class="dataTables_length" align="right">
                            <c:if test="${order.state ==310}">
                             <a title="同意退货" class="btn btn-sm btn-success agree" href="javascript:;"
                             	api-path="/afterSaleState/confirmReturn.do?returnId=${order.id}">
                            	<i class="ace-icon glyphicon glyphicon-send align-center bigger-125"></i>同意退货</a>
                           	 
                           	 <a title="拒绝退货" class="btn btn-sm btn-success disagree" href="javascript:;"
                             	api-path="/afterSaleState/refuseReturn.do?returnId=${order.id}">
                            	<i class="ace-icon glyphicon glyphicon-send align-center bigger-125"></i>拒绝退货</a>
                             </c:if>
                             
                            <c:if test="${order.state ==350}">
                             <a title="确认签收" class="btn btn-sm btn-success signed" href="javascript:;"
                             	api-path="/afterSaleState/signReturn.do?returnId=${order.id}">
                            	<i class="ace-icon glyphicon glyphicon-send align-center bigger-125"></i>确认签收</a>
                            </c:if>
                            <%--
                            <c:if test="${order.state ==360}">
                             <a title="确认退款" class="btn btn-sm btn-success send" href="javascript:;"
                             	api-path="/afterSaleState/payReturn.do?returnId=${order.id}">
                            ...
                            <!--   <a title="测试" class="btn btn-sm agree"
                               api-path="/afterSaleState/confirmReturn.do?id=${order.id}">
                                <i class="ace-icon fa fa-undo align-center bigger-125"></i>测试</a>  -->
                        </div>
                    </div>
                </div>
            </div>
...
<script type="text/javascript">
    ...
    ace.load_ajax_scripts(scripts, function () {
        ...
        $(".disagree,.send").on("click", function () {
         	var url = $(this).attr('api-path');
         	var title=$(this).attr('title');
            BootstrapDialog.show({
                title: title,
