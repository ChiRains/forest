...
<div class="row">
    <div class="col-xs-12">
        <!-- PAGE CONTENT BEGINS -->
        <form id="model-form" class="form-horizontal"  role="form" action="/admin/forestModular/editIndexModular.do">
			
            <c:forEach items="${result}" var="item" varStatus="current">
            
            <input type="hidden" name="modulars[${current.index}].code" value="${item.code}"/>
            <input type="hidden" name="modulars[${current.index}].type" value="${item.type}"/>
            		<div class="space-4"></div>
            <div class="form-group">
                <label class="col-sm-3 control-label no-padding-right" for="image"> ${item.name} </label>
                <div class="col-sm-9">
			    <span class="col-sm-5 no-padding block input-icon input-icon-right mr10">
			    	
					<input type="hidden" id="microImage${current.index}" name="modulars[${current.index}].value" value=""/>
					
                	<button type="button" mult="false" sid="microImage${current.index}" vid="pic${current.index}-pic-view" class="btn btn-sm btn-purple btn-upload-pic" upfrom="0">
                    <i class="ace-icon fa fa-upload"></i> 上 传
                	</button>
                    <ul sid="microImage${current.index}" id="pic${current.index}-pic-view" class="ace-thumbnails clearfix">
                    	<img src="${item.value}" width="120" height="80"/>
                    </ul>
		    </span>
                </div>
            </div> 
