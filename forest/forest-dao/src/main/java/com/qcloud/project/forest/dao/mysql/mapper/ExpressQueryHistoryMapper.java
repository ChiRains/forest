...
public interface ExpressQueryHistoryMapper {
	...
	@Insert("insert into `forest_express_query_history`(`id`,`expressName`,`expressNum`,`userId`,`time`)"
			+ " values(#{id},#{expressName},#{expressNum},#{userId},#{time})")
	...
	@Update("update `forest_express_query_history` set `expressName`=#{expressName},`expressNum`=#{expressNum},`userId`=#{userId},`time`=#{time} where `id`=#{id}")
